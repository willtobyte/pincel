...
cassette::cassette() {
  ...
  while (!remaining.empty()) {
    ...
    if (type == TYPE_NULL) {
      _data.try_emplace(std::string{key}, nullptr);
    } else if (type == TYPE_BOOL) {
      _data.try_emplace(std::string{key}, value == "1");
    } else if (type == TYPE_INT64) {
      int64_t v{};
      if (auto [ptr, ec] = std::from_chars(value.data(), value.data() + value.size(), v); ec == std::errc{}) {
        _data.try_emplace(std::string{key}, v);
      }
      ...
      _data.try_emplace(std::string{key}, decode_string(value));
    }
  }
}
...
static int cassette_set(lua_State* state) {
  ...
  switch (lua_type(state, 3)) {
    case LUA_TNUMBER: {
      const auto num = lua_tonumber(state, 3);
      double integral;
      if (std::modf(num, &integral) == 0.0) {
        if (num < 0) {
          instance.set(key, static_cast<int64_t>(integral));
        } else {
          instance.set(key, static_cast<uint64_t>(integral));
        }
      } else {
        instance.set(key, static_cast<double>(num));
      }
    } break;
    case LUA_TBOOLEAN:
      instance.set(key, static_cast<bool>(lua_toboolean(state, 3)));
      break;
    case LUA_TSTRING:
      instance.set(key, std::string_view{lua_tostring(state, 3)});
      break;
    default:
...
static int cassette_get(lua_State* state) {
  ...
  switch (lua_type(state, 3)) {
    case LUA_TBOOLEAN: {
      const auto fallback = static_cast<bool>(lua_toboolean(state, 3));
      lua_pushboolean(state, instance.get<bool>(key, fallback));
      return 1;
    }
    case LUA_TNUMBER: {
      const auto fallback = lua_tonumber(state, 3);
      double integral;
      if (std::modf(fallback, &integral) == 0.0) {
        if (fallback < 0) {
          lua_pushinteger(state, static_cast<lua_Integer>(instance.get<int64_t>(key, static_cast<int64_t>(integral))));
        } else {
          lua_pushinteger(state, static_cast<lua_Integer>(instance.get<uint64_t>(key, static_cast<uint64_t>(integral))));
        }
      } else {
        lua_pushnumber(state, instance.get<double>(key, fallback));
      }
      return 1;
    }
    case LUA_TSTRING: {
      const auto fallback = std::string_view{lua_tostring(state, 3)};
      if (const auto found = instance.find(key)) {
        if (const auto* v = std::get_if<std::string>(&*found)) {
          lua_pushstring(state, v->c_str());
          return 1;
        }
      }
      lua_pushstring(state, fallback.data());
      return 1;
    }
    default: {
...
static int cassette_index(lua_State* state) {
  const std::string_view method = luaL_checkstring(state, 2);

  if (method == "set")   return lua_pushcfunction(state, cassette_set), 1;
  if (method == "get")   return lua_pushcfunction(state, cassette_get), 1;
  if (method == "clear") return lua_pushcfunction(state, cassette_clear), 1;
